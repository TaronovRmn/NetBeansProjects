...
<html lang="en">
    ...
    <body>
        ...
        <div class="voting">
            <div class="wrapper">
                <div class="container">
                    <div class="htext">Голосование</div>
                    <div class="vote_container">
                        <c:choose>
                            <c:when test="${votetype == 'notlogged'}">
                                <div>${voting.getQuestion()}</div>
                                <c:forEach var="answer" items="${voting.getAnswers()}">
                                    <div>${answer.getText()} - ${answer.getCounter()} (${answer.getPercent()}%)</div>
                                </c:forEach>
                                <div>Авторизуйтесь, чтобы проголосовать.</div>
                            </c:when>
                            <c:when test="${votetype == 'voted'}">
                                <div>${voting.getQuestion()}</div>
                                <c:forEach var="answer" items="${voting.getAnswers()}">
                                    <div>${answer.getText()} - ${answer.getCounter()} (${answer.getPercent()}%)</div>
                                </c:forEach>
                            ...
                            <c:when test="${votetype == 'normal'}">
                                <form action="Vote" method="get">
                                    <div>${voting.getQuestion()}</div>
                                    <c:forEach var="answer" items="${voting.getAnswers()}">
                                        <div><label><input type="radio" name="vote" value="${answer.getId()}"/>${answer.getText()}</label></div>
                                            </c:forEach>
                                    <div><input type="submit" value="Проголосовать" />
                                </form>
                                <script>
                                    function setChecked() {
                                        var c = document.getElementsByName('vote');
                                        if (!c[0].checked)
                                            c[0].checked = true;
                                    }
                                    setChecked();
